...
public class MapRouterProcessFunc implements IComboMapRouterProcessFunc {
    ...
    @Override
    public <T> void routerVoidFunc(T t,Map<T, ? extends IVoidMapRouterProcessFunc> routeFuncMap) {
        routerVoidFunc(t,routeFuncMap,null);
    }


    public <T> void routerVoidFunc( T t,  Map<T, ? extends IVoidMapRouterProcessFunc> routeFuncMap, IExactFilter<T> matchFunc) {
    ...
    @Override
    public <T> T routerFunc(T t,  Map<T, ? extends IMapRouterProcessStateNodeFunc<T>> routeFuncMap) {
        return routerFunc(t,routeFuncMap,null);
    }

    @Override
    public <T> T routerFunc(T currentNode,  Map<T, ? extends IMapRouterProcessStateNodeFunc<T>> routeFuncMap, IExactFilter<T> matchFunc) {
